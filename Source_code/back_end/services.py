...
def to_float(value):
    # Form sends numbers as strings sometimes
    if value in (None, ""):
        return None
    try:
        return float(value)
...
class MeetupService:
    ...
    def createMeetup(self, data, base_url=""):
        # Save meetup first so participants have meetup_id
        ...
        # Creator needs distance too, so keep it as participant
        ...
        # Make first result now so result page is not empty
        self.generatePlan(event_code)
        return self.fetchMeetup(event_code)

    def joinMeetup(self, data):
        # After join, refresh the recommendation list
    ...
    def generatePlan(self, event_code):
        # Main planning steps live together here
    ...
    def fetchMeetup(self, event_code):
        # Frontend wants one big object here
        event_code = (event_code or "").upper()
        meetup = self.database.get_meetup_by_code(event_code)
        if not meetup:
            raise LookupError("Meetup not found")
    ...
    def addParticipantFromData(self, meetup, data):
        # If map picker gave coords, skip geocode
    ...
    def meetup_response(self, meetup, participants, result, share_link=None):
        # Keep old frontend fields and SDD result fields together
        data = meetup.to_dict()
