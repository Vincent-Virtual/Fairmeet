...
class MemoryDatabase:
    def __init__(self):
        # Quick local db when postgres is not up
        self.meetups = {}
        self.participants = {}
        self.venues = {}
        self.results = {}
...
class PostgresDatabase:
    ...
    def connect(self):
        # Import here so memory mode does not need psycopg2
        import psycopg2
    ...
    def init_schema(self):
        # Schema is small, easiest to run it on start
    ...
    def save_result(self, result):
        with self.connect() as conn:
            with conn.cursor() as cur:
                ...
                # Keep db ids on items so response matches rows
                for item in result.ranked_venues:
                    cur.execute(
                        """
...
def get_database():
    # Try postgres first, memory if local setup is not ready
